...
public class GlobalConfiguration : MonoBehaviour
{
    ...
    public void UpdateSimulatedTime()
    {
        ...
        foreach (var avatar in redirectedAvatars) {
            var rm = avatar.GetComponent<RedirectionManagerRDW>();
    ...
    public void CustomFunctionCalledInEveryStep() {        
        ...
        for (int i = 0; i < redirectedAvatars.Count; i++) {
            ...
            var r = redirectedAvatars[i].GetComponent<RedirectionManagerRDW>().redirector;
            if (r != null)
                r.GetPriority();
        }
        //large priority first
        avatarIdSortedFromHighPriorityToLow.Sort(delegate (int x, int y) {
            return redirectedAvatars[y].GetComponent<RedirectionManagerRDW>().priority.CompareTo(redirectedAvatars[x].GetComponent<RedirectionManagerRDW>().priority);
        });
    }
    ...
    {        
        ...
        for (var id = 0; id < redirectedAvatars.Count; id++)
        {
            ...
            var rm = redirectedAvatars[id].GetComponent<RedirectionManagerRDW>();
    ...
    public void MakeOneStepRedirection()
    {
        for (int i = 0; i < avatarIdSortedFromHighPriorityToLow.Count; i++)
        {
            redirectedAvatars[avatarIdSortedFromHighPriorityToLow[i]].GetComponent<RedirectionManagerRDW>().MakeOneStepRedirection();
        }
        ...
        foreach (var line in commands)
        {
            ...
            switch (split[0].ToLower())
            {
                ...
                case "newuser":
                    ...
                    break;
                case "redirector":
                    avatar.redirector = RedirectionManagerRDW.DecodeRedirector(split[2]);
                    break;
                case "resetter":
                    avatar.resetter = RedirectionManagerRDW.DecodeResetter(split[2]);
                    break;
                case "pathseedchoice":
    ...
    void EndExperiment(int endState)
    {
        ...
        if (passiveHaptics)
        {
            if (physicalTargets == null || physicalTargets.Count < redirectedAvatars.Count)
            {
                ...
                {
                    var posReal = Utilities.FlattenedPos2D(redirectedAvatars[id].GetComponent<RedirectionManagerRDW>().currPosReal);
                    var dirReal = Utilities.FlattenedPos2D(redirectedAvatars[id].GetComponent<RedirectionManagerRDW>().currDirReal);
    ...
    List<Dictionary<string, string>> GetExperimentDescriptor(ExperimentSetup setup)
    {
        ...
        for (int i = 0; i < setup.avatars.Count; i++)
        {
            ...
            descriptor["redirector"] = RedirectionManagerRDW.RedirectorToRedirectorChoice(setup.avatars[i].redirector).ToString();
            descriptor["resetter"] = RedirectionManagerRDW.ResetterToResetChoice(setup.avatars[i].resetter).ToString();
        }
